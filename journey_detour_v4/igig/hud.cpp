...
bool WorldToScreen(Point3 objectPos, Matrix4 worldMat, Matrix4 projMat,
 ...
 /* ImGui::Text("viewPos: %f %f %f %f", viewPos.getX(), viewPos.getY(),
  ...
  /*ImGui::Text("clipPos: %f %f %f %f", clipPos.getX(), clipPos.getY(),
              clipPos.getZ(), clipPos.getW());*/
...
static int luaC_updateCameraInfo(lua_State *L) {
  ...
  for (int index = 0; index <= 3; index++) {
    ...
    for (int index1 = 0; index1 <= 3; index1++) {
      ...
      lua_gettable(L, -2);
      IgIgHud::instance().worldMatrix[index][index1] =
  ...
  for (int index = 0; index <= 3; index++) {
    ...
    for (int index1 = 0; index1 <= 3; index1++) {
      lua_pushinteger(L, index1 + 1);
      lua_gettable(L, -2);
      IgIgHud::instance().projMatrix[index][index1] =
          luaL_checknumber(L, -1);
...
void IgIgHud::draw() {
  ...
  if (ImGui::Begin("Overlay", nullptr,
    ...
    //ImGui::Text("worldMat: %f %f %f %f", worldMatrix[0][0], worldMatrix[0][1],
    ...
    float minPos[3] = {192.84, 14.6392, 641.953};
    float maxPos[3] = {194.907, 15.6392, 644.02};
    ...
    for (auto ctx : hullRenderList) {
      for (int i = 0; i < ctx->edges.size(); i = i + 2) {
        ...
        // bool ws1 = WorldToScreen(lines[i][0], lines[i][1], lines[i][2],
        // viewMat,
        //                          cameraFov, cameraAspect, displaySize.x,
        //                          displaySize.y, &fromScreenX, &fromScreenY);
        // bool ws2 = WorldToScreen(lines[i][3], lines[i][4], lines[i][5],
        // viewMat,
        //                          cameraFov, cameraAspect, displaySize.x,
        ...
        bool ws1 = WorldToScreen(
            ...
            Float4ToMatrix4(IgIgHud::instance().worldMatrix),
            Float4ToMatrix4(IgIgHud::instance().projMatrix), displaySize.x,
            displaySize.y, &fromScreenX, &fromScreenY);
...
IgIgHud::IgIgHud() {
  ...
  LuaManager::instance().registerGlobal("luaC_addHullToHudRender", luaC_addHullToHudRender);
}
