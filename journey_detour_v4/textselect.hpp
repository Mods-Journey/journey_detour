...
// This class only works if the window only has text, and line wrapping is not
// supported. The window should also have the "NoMove" flag set so mouse drags
...
class TextSelect {
  ...
  // Selection bounds
  // In a selection, the start and end positions may not be in order (the user
  // can click and drag left/up which reverses start and end).
  CursorPos selectStart;
  CursorPos selectEnd;

  // Accessor functions to get line information
  // This class only knows about line numbers so it must be provided with
  // functions that give it text data.
  std::function<std::string_view(size_t)>
      getLineAtIdx;                    // Gets the string given a line number
...
public:
  ...
  // getLineAtIdx: Function taking a size_t (line number) and returning the
  // string in that line getNumLines: Function returning a size_t (total number
  // of lines of text)
